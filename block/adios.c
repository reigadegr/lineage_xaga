...
#define ADIOS_VERSION "3.0.0-rc1"
...
static int to_word_depth(struct blk_mq_hw_ctx *hctx, unsigned int qdepth) {
    if (!hctx->sched_tags) {
        pr_err("adios: hctx->sched_tags is NULL\n");
        return 0;
    }

    if (!hctx->sched_tags->bitmap_tags) {
        pr_err("adios: bitmap_tags is NULL\n");
        return 0;
    }
...
static void adios_limit_depth(unsigned int opf, struct blk_mq_alloc_data *data) {
    ...
    // 检查 elevator 结构是否存在
    if (!e) {
        pr_err("adios: elevator is null in adios_limit_depth\n");
        return;
    }
    // 检查 elevator_data 是否有效
    if (!e->elevator_data) {
        pr_err("adios: elevator_data is null in adios_limit_depth\n");
        return;
    }
    struct adios_data *ad = e->elevator_data;
    // 检查 async_depth 是否有效
    if (ad->async_depth == 0) {
        pr_err("adios: async_depth is 0 in adios_limit_depth\n");
        return;
    }

    // Do not throttle synchronous reads
    if (op_is_sync(opf) && !op_is_write(opf)) {
        pr_err("adios: skipping synchronous read in adios_limit_depth\n");
        return;
    }

    data->shallow_depth = to_word_depth(data->hctx, ad->async_depth);
    pr_err("adios: adios_limit_depth set shallow depth successfully\n");
}
...
static void insert_request_pre_stability(struct blk_mq_hw_ctx *hctx,
	...
	spin_lock_irqsave(&ad->pq_lock, _flags);
	...
	spin_unlock_irqrestore(&ad->pq_lock, _flags);
...
static void adios_insert_requests(struct blk_mq_hw_ctx *hctx,
	...
	LIST_HEAD(free);
	unsigned long _flags;
	spin_lock_irqsave(&ad->lock, _flags);
	do {	
	...
	spin_unlock_irqrestore(&ad->lock, _flags);

	// blk_mq_free_requests(&free);
	struct request *next;
    list_for_each_entry_safe(rq, next, &free, queuelist)
...
static struct request *dispatch_from_bq(struct adios_data *ad) {
	...
	spin_lock_irqsave(&ad->bq_lock, flag);
...
again:
	...
	if (rq) {
		...
		spin_unlock_irqrestore(&ad->bq_lock, flag);
		return rq;
	}
...
static struct request *dispatch_from_pq(struct adios_data *ad) {
	...
	spin_lock_irqsave(&ad->pq_lock, flag);
	...
	if (unlikely(list_empty(q))) {
	    spin_unlock_irqrestore(&ad->pq_lock, flag);
	    return NULL;	    
	}
...
static ssize_t adios_read_priority_store(
	...
	spin_lock_irqsave(&ad->lock, flag);
	ad->dl_prio[0] = prio;
	ad->dl_bias = 0;
	spin_unlock_irqrestore(&ad->lock, flag);
